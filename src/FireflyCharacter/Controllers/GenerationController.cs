...
namespace FireflyCharacter.Controllers
{
	...
	public class GenerationController : Controller
	{
		...
		public GenerationController(IHostingEnvironment env)
		{
			pathRoot = env.WebRoot;
		}

		[HttpPost("character")]
		public IActionResult Character([FromBody] Models.FireflyCharacter character)
		{
			var source = Path.Combine(pathRoot, "Content\\InteractiveCrewAndShipSheets_MWP_DTRPG.pdf");
			...
			Action<string, string> setText = (fieldName, value) => pdfStamper.AcroFields.SetField(fieldName, value);
			Action<string> check = (fieldName) => pdfStamper.AcroFields.SetField(fieldName, "Yes");

			//PdfDocument outputDocument;
			//using (var inStream = new MemoryStream(System.IO.File.ReadAllBytes(source)))
			//{
			//	outputDocument = PdfReader.Open(inStream, PdfDocumentOpenMode.Modify);
			//}
			//outputDocument.Pages.RemoveAt(2);
			//outputDocument.Pages.RemoveAt(0);


			//if (outputDocument.AcroForm.Elements.ContainsKey("/NeedAppearances"))
			//{
			//	outputDocument.AcroForm.Elements["/NeedAppearances"] = new PdfSharp.Pdf.PdfBoolean(true);
			//}
			//else
			//{
			//	outputDocument.AcroForm.Elements.Add("/NeedAppearances", new PdfSharp.Pdf.PdfBoolean(true));
			//}

			//Exception last;
			//Action<string, string> setText = (fieldName, text) =>
			//{
			//	try
			//	{
			//		((PdfTextField)(outputDocument.AcroForm.Fields[fieldName])).Text = text;
			//	}
			//	catch (Exception ex)
			//	{
			//		last = ex;
			//             }
			//};
			//Action<string> check = (fieldName) =>
			//{
			//	try
			//	{
			//		((PdfCheckBoxField)(outputDocument.AcroForm.Fields[fieldName])).Checked = true;
			//	}
			//	catch (Exception ex)
			//	{
			//		last = ex;
			...
			setText(NamePdfField, character.Bio.Name);
			setText(DescriptionPdfField, character.Bio.Gender.ToString("g"));
			setText(BackgroundPdfField, character.Bio.System);
			...
			for (var i = 0; i < 3; i++)
			{
				setText(distinctions[i].Name, character.Distinctions[i].Name);
				for (var j = 0; j < 2; j++)
				{
					setText(distinctions[i].Triggers[j].Trigger, character.Distinctions[i].Triggers[j].Name + ": " + character.Distinctions[i].Triggers[j].Description);
			...
			foreach (var skill in character.Skills)
			{
				...
				{
					setText(skills[skill.Name].Speciality, skill.Speciality ?? "--Purchased/Undecided--");
				}
				check(skills[skill.Name].BySides[skill.Rating.Sides]);
			}
			foreach (var skillName in skills.Keys.Except(character.Skills.Select(skill => skill.Name)))
			{
				check(skills[skillName].BySides[4]);
			}

			setText(SignatureAssetsPdfField, string.Join("\n", from sa in character.SignatureAssets
		...
		{
			return File(pdfs[id], "application/pdf");
		}
	}
}
